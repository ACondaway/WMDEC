# SD3.5 Phase 2 — Texture Control Training
# Trains: TextureControlBranch + continues PerceiverResampler + ResidualMLPAdaptor
# Frozen: SD3.5 MMDiT, VAE, Qwen Visual Encoder
#
# Initialises Phase 1 weights from a checkpoint before training.
#
# Usage (single node, 8×H100):
#   torchrun --nproc_per_node=8 training/train_sd35_phase2.py --config configs/sd35_phase2.yaml \
#       --phase1_ckpt outputs/sd35_phase1/phase1_latest.pt
#
# Usage (multi-node):
#   torchrun --nnodes=4 --nproc_per_node=8 --node_rank=<RANK> \
#       --master_addr=<MASTER_IP> --master_port=29500 \
#       training/train_sd35_phase2.py --config configs/sd35_phase2.yaml \
#       --phase1_ckpt outputs/sd35_phase1/phase1_latest.pt
...
# ---------------------------------------------------------------------------
# CFG dropout (Phase 2 — image + control)
# ---------------------------------------------------------------------------

def _apply_cfg_dropout_phase2(
    ...
    """
    Returns (qwen_features, latent, force_drop_flags) with per-sample CFG dropout.

    force_drop_flags : (B,) bool — True means force TextureControlBranch to zero.
    The texture control is dropped together with the image condition (drop_all case).
    """
    B = qwen_features.shape[0]
    r = torch.rand(B, device=qwen_features.device)

    drop_img = (r >= p_keep) & (r < p_keep + p_drop_img)
    drop_all = r >= (p_keep + p_drop_img + p_drop_txt)

    qwen_out = qwen_features.clone()
    qwen_out[drop_img | drop_all] = 0.0

    # For texture control: only zero-out on drop_all
    force_drop = drop_all

    return qwen_out, latent, force_drop
...
def save_checkpoint(
    ...
    ckpt = {
        ...
        "control":   model.control.state_dict(),
        "optimizer": optimizer.state_dict(),
        "scaler":    scaler.state_dict(),
    }
    path = os.path.join(output_dir, f"phase2_step{step:07d}.pt")
    torch.save(ckpt, path)
    latest = os.path.join(output_dir, "phase2_latest.pt")
...
def load_phase1_weights(path: str, model: SD35SemanticModel) -> None:
    ckpt = torch.load(path, map_location="cpu", weights_only=False)
    model.resampler.load_state_dict(ckpt["resampler"])
    model.adaptor.load_state_dict(ckpt["adaptor"])
    print(f"Phase 1 weights loaded from {path}")
...
def main():
    parser = argparse.ArgumentParser(description="SD3.5 Phase 2 — Texture Control")
    parser.add_argument("--config",      required=True, help="Path to sd35_phase2.yaml")
    parser.add_argument("--phase1_ckpt", required=True, help="Phase 1 checkpoint to initialise from")
    parser.add_argument("--resume",      default=None,  help="Phase 2 checkpoint to resume from")
    ...
    # Load Phase 1 weights, then switch to Phase 2 training mode
    load_phase1_weights(args.phase1_ckpt, model)
    model.set_phase2()
    ...
    output_dir  = cfg.get("output_dir", "outputs/sd35_phase2")

    warmup_steps = cfg.get("warmup_steps", 500)
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer,
        max_lr=cfg.get("lr", 5e-5),
    ...
    while step < total_steps:
        ...
        for batch in loader:
            ...
            # Encode image to latent for texture control (no grad needed)
            with torch.no_grad():
                latent = model.encode_image_to_latent(image)

            # CFG condition dropout
            qwen_features, latent, force_drop = _apply_cfg_dropout_phase2(
                qwen_features, latent
            )
